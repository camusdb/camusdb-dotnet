...
- **Precedence.** An explicit transaction stays pinned to the endpoint it started on — advice may inform the next transaction, never relocate a live one. A learned route pointing at a quarantined endpoint falls back to rotation. Routing adds no retry of any kind.
- **Where a transaction starts.** With routing on, `BEGIN` is deferred: `BeginTransaction` sends nothing, and the transaction's first statement starts it on that statement's learned endpoint, so a transaction whose first statement touches table `T` runs where `T`'s leader is. Every later statement and the finalize stay on that endpoint. Statements inside the transaction still negotiate and learn, for the next transaction's benefit. To pick the endpoint from a later, hotter statement instead, pass its exact SQL as `CamusTransactionOptions.Affinity`; `BEGIN` is then sent at once to that statement's learned endpoint (rotation when nothing is learned). With routing off, `BEGIN` is sent inside `BeginTransaction` on the pool's rotation, exactly as before.
- **What the deferral changes for a caller.** In routed mode `CamusTransaction.TxnIdPT`, `TxnIdCounter` and `TransactionId` read as zero until the first statement (`IsStarted` says which state the transaction is in), and a failure to begin surfaces from that first statement with the same exception `BeginTransaction` would have thrown. A transaction that runs no statement is started by its `Commit`/`Rollback`, on rotation, so the server sees the same `BEGIN`-then-finalize pair. Set locking and isolation through `BeginTransactionAsync(CamusTransactionOptions)` or the connection-string defaults, not with a `SET TRANSACTION` statement: as the first statement it carries no routing advice and starts the transaction on rotation.
...
- **A gRPC stream is rotated when the token is renewed.** With `Protocol=grpc`, statements ride on long-lived `BatchExecute` streams, and a stream can present a token only once, when it opens. The driver does not rely on a stream outliving that token. On the first statement after a renewal, each stream is replaced by one opened under the new token. A transaction cannot change streams, so the old stream stays open for the transactions that began on it, takes no new work, and closes when the last of them commits or rolls back — or after five minutes, so that an abandoned transaction cannot hold it, and its locks, open. While its stream drains, such a transaction runs its statements unprepared. Nothing is needed from the caller, and no statement fails because of a rotation. A token supplied with `AccessToken=` is never renewed, so its streams are never rotated.
...
The stream pool is sized by `ChannelPoolSize` (default `2`). It is *not* a cap on in-flight transactions — many transactions hash onto the same streams and interleave — so the default suits most workloads; raise it when many long-running streaming queries against one endpoint would otherwise queue behind each other on a shared stream. `CoalescingThreshold` and `CoalescingDelay` trade latency for fuller frames when a burst of operations arrives together on one connection; the delay is off by default and is never applied after a single-op drain, because a caller in request/response lockstep would pay it on every round trip (measured: ~3.5 ms per statement, 12% of throughput at 128 workers).

**Stream frames.** When several operations wait for the same stream at the same moment, the client writes them as one stream message, a *frame*, and it reads a result that the server packed in the same manner. This removes the fixed cost that each stream message has on the two ends. A frame never waits for more operations: an operation that is alone travels as the same single message as before, so a client with one caller sees no added latency. Frames are negotiated per stream and are safe against an older server: the server announces support in the response headers of the stream (`camusdb-batch-frames`), and the client sends a frame only after it sees that announcement on that same stream. A server that announces nothing receives one message per operation, as before. In the other direction, the client tells the server that it reads response frames with the request header `camusdb-batch-frames-accept` when the stream opens, so a multi-row result from one caller can arrive as one message; an older server ignores that header. A frame is a transport optimization only. It gives no atomicity and no ordering that the stream does not already give; use a transaction for that. If a frame write fails, each operation in it fails with the transport error and the usual retry contract applies; the client never sends a frame again. Set `RequestFrames=false` to write one operation per message to every server, for A/B measurements or as a kill switch.
